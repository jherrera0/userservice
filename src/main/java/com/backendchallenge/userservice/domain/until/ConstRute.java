package com.backendchallenge.userservice.domain.until;

public class ConstRute {
    public static final String USER_REST_RUTE = "/user";
    public static final String CREATE_OWNER_RUTE = "/CreateOwner";
    public static final String FIND_OWNER_BY_ID_RUTE = "/FindOwnerById";
    public static final String AUTH = "/auth";
    public static final String LOGIN = "/login";
    public static final String CREATE_EMPLOYEE_RUTE = "/CreateEmployee";
    public static final String CREATE_CLIENT_RUTE = "/CreateClient";

    private ConstRute() {
    }
}
